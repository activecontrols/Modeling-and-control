function [xset, uset, tset] = get_trajectory(numPoints, xcritset, ucritset, constants, MOI)
    mass = constants(1);
    grav = constants(3);
    ...
    for i = 1:(numCrits - 1)
        [xset(:, (1+numPoints*(i-1)):numPoints*i), uset(:, (1+numPoints*(i-1)):numPoints*i), tset(1, (1+numPoints*(i-1)):numPoints*i)] = get_segment_traj(numPoints, tSegs(i), tSegs(i+1), xcritset(:, i), xcritset(:, i+1), ucritset(:, i+1), Q, R, constants, MOI);
