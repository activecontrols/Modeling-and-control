function [x_set, u_set, t_set, K_set, tSegs, startTime, stopTime] = get_trajectory_OLD(numPoints, tf, xcritset, ucritset, constants, MOI, limits, throttleConsts)
    ...
    for i = 1:(numCrits - 1)
        if i > 1
            [xset(:, (1+numPoints*(i-1)):numPoints*i), uset(:, (1+numPoints*(i-1)):numPoints*i), t_set(1, (1+numPoints*(i-1)):numPoints*i), K_set(:, :, i)] = get_segment_traj_OLD(numPoints, tSegs(i), tSegs(i+1), xset(:, numPoints*(i-1)), xcritset(:, i+1), ucritset(:, i+1), Q, R, constants, MOI, limits, throttleConsts);
        else
            [xset(:, (1+numPoints*(i-1)):numPoints*i), uset(:, (1+numPoints*(i-1)):numPoints*i), t_set(1, (1+numPoints*(i-1)):numPoints*i), K_set(:, :, i)] = get_segment_traj_OLD(numPoints, tSegs(i), tSegs(i+1), xcritset(:, i), xcritset(:, i+1), ucritset(:, i+1), Q, R, constants, MOI, limits, throttleConsts);
        end
